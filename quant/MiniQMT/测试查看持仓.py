...
class MyStrategy(XtQuantTraderCallback):
    def __init__(self):
        ...
        for _, pos in positions.iterrows():  # 遍历 DataFrame 的行 (Series)
            print(pos)
        ...
        def get_position(self, stock_code=None):
            """
            查询持仓信息
            :param stock_code: 股票代码，None表示查询所有持仓
            :return: 单个持仓的字典或所有持仓的DataFrame
            """
            if stock_code:
                # 查询指定股票持仓
                ...
                if position:
                    return {
                        ...
                        '持仓量': position.volume,  # 当前持仓量
                        '可用量': position.can_use_volume,  # 可用持仓量
                        '冻结量': position.frozen_volume,  # 冻结持仓量
                ...
                if positions:
                    ...
                    for pos in positions:
                        pos_list.append({
                            ...
                            '成本价': pos.open_price,
                            '持仓量': pos.volume,
                            '可用量': pos.can_use_volume,
                            '冻结量': pos.frozen_volume,
                            '市值': pos.market_value
                        })
                    return pd.DataFrame(pos_list)
                return pd.DataFrame()  # 空仓时返回空DataFrame
