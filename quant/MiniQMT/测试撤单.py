        ...
        for _, order in orders.iterrows():  # 遍历 DataFrame 的行 (Series)
            ...
            if order.状态 == 50 or order.状态 == 55:
                ...
                if (now - order_time).seconds > 10 and order.委托编号:
                    ...
                    print('撤单合同编号', order.委托编号, '股票代码', order.证券代码)

    def query_orders(self, order_id=None):
        """
        查询订单状态
        :param order_id: 指定订单号查询，None表示查询所有订单
        :return: 单个订单字典或所有订单DataFrame
        """
        if order_id:
            # 查询指定订单
            order = self.trader.query_stock_order(self.trader, order_id)  # 注意这里的传参，原图似乎写的是self.trader，通常应为self.acc
            if order:
                return {
                    '委托编号': order.order_id,  # 订单ID
                    '证券代码': order.stock_code,  # 股票代码
                    '委托价格': order.price,  # 委托价格
                    '委托量': order.order_volume,  # 委托数量
                    '成交量': order.traded_volume,  # 已成交量
                    '成交均价': order.traded_price,  # 成交均价
                    '状态': order.order_status,  # 订单状态
